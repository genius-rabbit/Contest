...
def win_rate(file,first):
    ...
    while(x<0.7):
        for index in range(0, len(first)):
            ...
            rate.append(0.2877559 + (win_rate_home[first[index][1]] - win_rate_away[first[index][0]]) *0.496 + (
            res[first[index][1]] - res[first[index][0]]) * 0.04649)
        ...
        for index in range(0, len(file)):
            sum = sum + abs(x + (win_rate_home[file[index][1]] - win_rate_away[file[index][0]]) * x + (
            res[file[index][1]] - res[file[index][0]]) * 0.020149 - (file[index][3] - file[index][2]) / file[index][3])
